...
def test_validate_number_positive_decimal():
    assert InputValidator.validate_number(
        123.456, config) == Decimal('123.456').normalize()
...
def test_validate_number_negative_decimal():
    assert InputValidator.validate_number(-789.123,
                                          config) == Decimal('-789.123').normalize()
...
def test_validate_number_negative_string_decimal():
    assert InputValidator.validate_number(
        "-789.123", config) == Decimal('-789.123').normalize()
